...
//  Helpers
// -------------------------------
const toSec  = (ms) => (ms == null ? null : +(ms / 1000).toFixed(2));
const fmtS   = (s)  => (s == null ? null : `${s.toFixed(2)}s`);
...
//  Colores / Tendencia
// -------------------------------
export function colorFor(key, val) {
...
export function trendFor(key, curr, prev) {
...
//  - tbt: milisegundos
// -----------------------------------------
export function readMetrics(doc) {
...
// -----------------------------------------------------
export function extractOpportunities(doc) {
  const lhr = getLHR(doc);
  const audits = lhr?.audits || {};
  if (!audits || !Object.keys(audits).length) return [];

  const out = [];
  const secLabel = (ms) => `${(ms / 1000).toFixed(1)}s`;
  const push = ({ id, title, savingMs = 0, savingBytes = 0, recommendation = null }) => {
    const savingsLabel = savingMs > 0 ? secLabel(savingMs) : (savingBytes > 0 ? fmtB(savingBytes) : null);
    const impactScore = (savingMs || 0) + (savingBytes || 0) / 1024; // ranking simple
    if (!savingsLabel && !recommendation) return; // nada útil
    out.push({ id, title: title || audits[id]?.title || id, savingsLabel, impactScore, recommendation });
  };
  ...
  for (const [id, a] of Object.entries(audits)) {
    ...
    const ms = (typeof d.overallSavingsMs === 'number' ? d.overallSavingsMs : 0)
      + sum(items.map(it => it.wastedMs || it.overallSavingsMs || 0));
    const bytes = (typeof d.overallSavingsBytes === 'number' ? d.overallSavingsBytes : 0)
      + sum(items.map(it => it.wastedBytes || it.overallSavingsBytes || 0));
    if ((ms > 0 || bytes > 0) && a.score !== 1 && a.scoreDisplayMode !== 'notApplicable') {
      push({ id, title: a.title, savingMs: ms, savingBytes: bytes });
    }
  }
  ...
  const ensure = (id, fn) => { if (!out.find(o => o.id === id)) fn(); };

  ensure("unused-javascript", () => {
    const a = audits["unused-javascript"]; if (!a || a.score === 1) return;
    const bytes = a.details?.items ? sum(a.details.items.map(it => it.wastedBytes || 0)) : 0;
  ...
  if (out.length === 0) {
    ...
    if (audits["largest-contentful-paint"]?.numericValue != null) {
      addMetric("largest-contentful-paint",
    ...
    if (ttfbAudit?.numericValue != null) {
      addMetric("server-response-time",
...
// -----------------------------------------
export function packMetrics(curr, prev) {
  ...
  return order.map((key) => {
    ...
    else if (raw != null)                     display = fmtS(raw);
...
// -----------------------------------------
export function debugOpportunities(doc) {
  const lhr = getLHR(doc);
  const audits = lhr?.audits || {};
