...
type PlanItem = {
  ...
  savingsLabel?: string;            // ej: "1.2s", "300KB", etc.
  type: "error" | "improvement";
  severity?: "critical" | "info";   // opcional, no siempre usado
  impactScore?: number;             // fallback para ordenar
};

type ExtendedPlanItem = PlanItem & {
  _k: string;
  sev: { lvl: SeverityLevel; Icon: IconComp; bar: number };
};
...
type Props = {
  opportunities?: PlanItem[];
  performance?: number | null;  // <- acepta number y también null/undefined
};
...
export default function ActionPlanPanel({
  ...
  const toggleDone  = (k: string) => setDone((s) => ({ ...s, [k]: !s[k] }));
  ...
  const copyChecklist = async () => {
    try {
      await navigator.clipboard.writeText(pendingMarkdown());
  ...
  const Card: React.FC<{ o: ExtendedPlanItem }> = ({ o }) => {
