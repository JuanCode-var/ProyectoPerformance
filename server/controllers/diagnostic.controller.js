...
function buildThresholdOpps(curr) {
  const opps = [];
  const ms = (s) => Math.max(0, Math.round(s * 1000));
  const push = (id, title, reco, savingMs = null) => {
    opps.push({
      id,
      title,
  ...
  if (curr.tbt != null && curr.tbt > THRESHOLDS.tbt.green) {
    push(
      ...
      Math.max(0, curr.tbt - THRESHOLDS.tbt.green)
    );
  }
...
export async function getProcessedByUrl(req, res) {
  try {
    ...
    const sameType = docs.filter((d) => d.type === docs[0].type);
